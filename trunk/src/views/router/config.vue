        ...
        <el-table-column label="操作" width="250">
          <template slot-scope="scope">
            ...
            <el-button
              type="text"
              size="small"
              @click="addItem('auth', scope.row)"
              v-if="(!scope.row.sublist || scope.row.sublist.length === 0) && scope.row.code"
            >新增按钮</el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  methods: {
    ...
    tableLoad(tree, treeNode, resolve) {
      ...
      if (tree.code) {
        ...
        let appId = tree.id;
        this.request(this.api.system.menu_getListMap, { appId: appId }).then(
          data => {
            console.log(data);
            if (data && data.length) {
