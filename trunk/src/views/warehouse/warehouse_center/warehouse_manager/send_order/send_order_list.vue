  ...
  <div>
    <div class="search_div">
      <el-form ref="formData" :model="signData" label-width="110px" :inline="true">
        ...
        <el-form-item label="同步状态：" prop="warehouseCode">
          <el-select v-model="signData.warehouseCode" style="width:185px;">
            <el-option
              v-for="item in warehouseData"
              :key="item.id"
          ...
          <template slot-scope="scope">
            ...
            <el-dropdown @command="handleCommand">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item :command="saveCurrentItemData('asyncStatus',scope.row)">单据同步</el-dropdown-item>
                <el-dropdown-item :command="saveCurrentItemData('obsolete',scope.row)">作废</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
...
export default {
  ...
  methods: {
    getSendOrderTableData() {
      ...
      // }
      this.request(
        this.api.warehouse.warehouseBase_deliveryOrder_list,
        this.signData
      ).then(res => {
        console.log(res);
    ...
    addBtnClickEvent() {
      ...
      this.$router.push({
        path:
          "/warehouse/warehouse_center/warehouse_manager/send_order/send_order_add"
      });
    },
    //导出选中的数据
    ...
    checkBtnClick(row) {
      this.$router.push({
        path:
          "/warehouse/warehouse_center/warehouse_manager/send_order/send_order_detail",
        query: {
          orderSn: row.orderSn
        }
      });
    ...
    handleCommand(data) {
      ...
      switch (data.desc) {
        ...
        case "obsolete":
          //作废事件
          this.obsoleteBtnClick();
          break;
      }
    },
    saveCurrentItemData(desc, row) {
      return {
        desc,
        row
      };
