  ...
  <div class="base">
    <div class="middle-from">
      ...
      >
        ...
        <el-form-item label="库存锁定状态：" prop="businessStatusList">
          <el-select v-model="formInline.businessStatusList" placeholder="请选择" :multiple="true">
            <el-option label="待发货" value="05"></el-option>
            <el-option label="已发货" value="06"></el-option>
            <el-option label="已完成" value="08"></el-option>
        ...
        <el-form-item label="同步状态：" prop="businessStatusList">
          <el-select v-model="formInline.businessStatusList" placeholder="请选择" :multiple="true">
            <el-option label="待发货" value="05"></el-option>
            <el-option label="已发货" value="06"></el-option>
            <el-option label="已完成" value="08"></el-option>
    ...
    <div class="table">
      ...
      >批量审核</el-button>
      <el-button plain @click="batchLockLibraryEvent" size="small">批量锁库</el-button>
      <el-button
        plain
        @click="exportLogisticsInfoEvent"
        size="small"
        v-auth="codeData"
        data-code="COSO_IMPORT"
      >导出物流信息</el-button>
      ...
      <template>
        ...
        >
          ...
          <el-table-column prop="orderStatus" label="库存锁定状态" width="120" align="center">
            <template slot-scope="scope">
              <span>{{scope.row | filterReviewed}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="orderStatus" label="同步状态" width="80" align="center">
            <template slot-scope="scope">
              <span>{{scope.row | filterReviewed}}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="amount" label="商品总数量" width="180" align="center">
            <template slot-scope="scope">
...
export default {
  ...
  methods: {
      ...
      this.request(this.api.order.channelSaleOrder_list, this.formInline).then(
        res => {
          if (res.data.length == 0 && res.totalCount > 0) {
            ...
            this.request(
              this.api.order.channelSaleOrder_list,
              this.formInline
            ).then(res => {
              //获取总的页数
    ...
    //批量锁库事件
    batchLockLibraryEvent() {
      console.log("批量锁库。。。。");
    },
    //导出物流信息事件
    exportLogisticsInfoEvent() {
      console.log("导出物流信息。。。");
    },
    //文件上传相关的事件
    // 手动上传
    ...
    submitUpload() {
      ...
      formdata.append(
        "channelSaleOrderImport",
        this.fileData.get("OrderTableList")
      );
      formdata.append(
        "channelSaleOrderImport",
        this.fileData.get("BabyTableList")
      );
