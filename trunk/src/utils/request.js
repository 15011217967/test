...
function auto(obj, params = {}, noLoading) {
  ...
  return new Promise((resolve, reject) => {
    switch (rst.method) {
      case "POST":
      service.post(rst.url, params, { ...rst }).then(resp => {
        if(
          // 如果有返回结果
          resp &&
          // 并且有data数据
          ...
          // 并且页数大于1
          params.pageIndex > 1 &&
          // 并且data.length等于0，就说明本页数据为空，需要重新获取上一页的数据
          (resp.data.data && resp.data.data.length === 0 || resp.data && resp.data.length === 0)
        ){
          params.pageIndex --;
          service.post(rst.url, params, { ...rst }).then(resp => resolve(resp.data), err => reject(err)).catch(err => reject(err));
        }else{
          resolve(resp.data);
        };
